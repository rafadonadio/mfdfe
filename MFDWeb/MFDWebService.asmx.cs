...
using Entities;
using BusinessLogic;
using MFDWebService.ar.gov.afip.wswhomo;
using MFDWebService.ar.gov.afip.wsaahomo;
using MFDWebService.Clases.Adapters;
using Tools.XML.Generator;
using System.Globalization;

namespace MFDWebService
{
	/// <summary>
	/// Summary description for MFDWebService
	/// </summary>
	...
	public class MFDWebService : System.Web.Services.WebService
	{
		...
		public byte[] GetRecipientCertificateFromStore(string recipientName)
		{
			...
			if (certColl.Count == 0)
			{
				...
				for (int i = 0; i < certColl.Count; i++ )
				{

					X509Certificate2 cer = certColl[i];
		...
		private  X509Certificate2 GetSenderCertificate(Empresas empresa)
		{
			...
			//Orig: X509Store storeAddressBook = new X509Store(StoreName.My, StoreLocation.CurrentUser);
			...
			/*X509Certificate2Collection certColl = storeAddressBook.Certificates;
			...
			if (storeAddressBook.Certificates.Count > 0)
			{
				...
				{
					X509Certificate2 cer = certColl[i];
					if(cer.GetName().Contains("CN="+empresa.SenderName)){
						result = cer;
						break;
					}
					string name = cer.GetName();
				...
				throw new Exception("No se encontraron certificados. :P empresa.SenderName:  " + empresa.SenderName);
			*/
